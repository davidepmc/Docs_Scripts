...
select 
        ...
        round(mod(to_char(sysdate,'HH24')+((((TOTALWORK-SOFAR)*(value/1024/1024))/(sofar*(value/1024)/1024/decode(elapsed_seconds,0,1,elapsed_seconds)))/60/60),60))||':'||trunc(mod(
to_char(sysdate,'MI')+(((TOTALWORK-SOFAR)*(value/1024/1024))/(sofar*(value/1024)/1024/decode(elapsed_seconds,0,1,elapsed_seconds)))/60,60)) final
