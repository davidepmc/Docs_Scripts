WITH cuando AS
 ...
 (SELECT /*+ MATERIALIZE */ -- para tratar la query como una vista materializada
  ...
  --AND st.sql_id NOT IN ('') -->> Filtro para blackout de SQL_IDs
   ...
   (select sql_id, phv
    from sql_online
    where (   (Elap_x_ej_ms > 500 and ej_onl>10)
   ...
   , diff_new_phv as
   (
    select /*+ MATERIALIZE use_hash (bpa) leading (pn so bpa)*/ pn.sql_id, pn.phv, to_char(bpa.time_mcsecs) AVG_MS_ONL,
        so.Elap_x_ej_ms,  so.Elap_x_ej_ms-bpa.time_mcsecs diff_ms,
        round(((so.Elap_x_ej_ms-bpa.time_mcsecs)/bpa.time_mcsecs)*100,1) pct_diff,null phvs_onl,null ej_onl
    from phv_noregistradas pn, best_phv_awr bpa, sql_online so
