...
#######################################
##				     ##
## QUERY PARA VER OPERACIONES EN SGA ##
##				     ##
#######################################
...
select fsnap, inst_id, time, oper, comp, param, MB_TARGET, status
from (
  select 
  sfst.SNAP_ID fsnap,
  sfst.instance_number inst_id,
  sfst.start_time time,
  sfst.oper_type oper,
  sfst.component comp,
  sfst.parameter param,
  --sum(sfst.final_size-sfst.initial_size)/1024/1024/1024 MB,
  sfst.target_size/1024/1024 MB_TARGET,
  sfst.status
  from 
  DBA_HIST_MEMORY_RESIZE_OPS sfst, DBA_HIST_MEMORY_RESIZE_OPS ssfst
  where sfst.instance_number = ssfst.instance_number
  group by sfst.snap_id, sfst.instance_number,sfst.start_time,sfst.oper_type,sfst.component,sfst.parameter, sfst.target_size, sfst.status)
  order by 3,2
;
...
break on fsnap on ssnap on inicio on fin skip 1;
  ...
  select fsnap,
  ssnap,
  to_char(finterval, 'DD/MM/YY hh24:mi') inicio,
  to_char(sinterval, 'hh24:mi') fin,
  event_name, 
  class,
...
--ACCEPT nodo_1 PROMPT 'Introduce el inst_id 1:'
--ACCEPT nodo_2 PROMPT 'Introduce el inst_id 2:'
...
ACCEPT FECHA PROMPT 'Introduce el día a mostrar (DD/MM/YY):'
  ...
  round(total_wait,3) total_wait from
  (
    select instance_number inst_id, finterval, sinterval, event_name, class, total_wait
    ,row_number() OVER (PARTITION BY fsnap ORDER BY  (total_wait) desc ) pos
      FROM (
      select fst.instance_number,
      fst.snap_id fsnap, 
      sst.snap_id ssnap,
      sfst.END_INTERVAL_TIME finterval,
      ssst.END_INTERVAL_TIME sinterval, 
      ...
      (sst.TIME_WAITED_MICRO-fst.TIME_WAITED_MICRO)/1000000 total_wait
      from DBA_HIST_SYSTEM_EVENT fst, DBA_HIST_SYSTEM_EVENT sst, DBA_HIST_SNAPSHOT sfst, DBA_HIST_SNAPSHOT ssst
      where fst.instance_number=&&nodo_1
	  and sst.instance_number=&&nodo_1
	  and sfst.instance_number=&&nodo_1
	  and ssst.instance_number=&&nodo_1
	  and fst.event_name = sst.event_name
      and fst.WAIT_CLASS=sst.WAIT_CLASS
      and fst.WAIT_CLASS <> 'Idle'
      and fst.snap_id=sst.snap_id-1
      and fst.snap_id=sfst.snap_id
      ...
      (sst.TIME_WAITED_MICRO-fst.TIME_WAITED_MICRO)/1000000 total_wait
      from DBA_HIST_SYSTEM_EVENT fst, DBA_HIST_SYSTEM_EVENT sst, DBA_HIST_SNAPSHOT sfst, DBA_HIST_SNAPSHOT ssst
      where fst.instance_number=&&nodo_2
	  and sst.instance_number=&&nodo_2
	  and sfst.instance_number=&&nodo_2
	  and ssst.instance_number=&&nodo_2  
	  and fst.event_name = sst.event_name
      and fst.WAIT_CLASS=sst.WAIT_CLASS
      and fst.WAIT_CLASS <> 'Idle'
      and fst.snap_id=sst.snap_id-1
...
select inst_id, fsnap, ssnap, inicio, fin, HASH, SQL_ID, EXECs, SS_CPU, SS, SC_per_EXEC, PHY_RDS, BUFF_GET
    FROM
    (
      select inst_id, fsnap,ssnap,inicio, fin, HASH, SQL_ID, EXECS, SS_CPU, SS, SC_per_EXEC, PHY_RDS, BUFF_GET
      ...
      FROM (
	    select 
	    trunc(sfst.instance_number,1) inst_id,
	    sfst.snap_id-1 fsnap, 
	    sfst.snap_id ssnap,
	    to_char(sfst.BEGIN_INTERVAL_TIME, 'DD/MM/YY HH24:MI') inicio,
	    to_char(sfst.END_INTERVAL_TIME, 'HH24:MI') fin,
  ...
  WHERE pos <= 5
  order by fsnap, &ORDENAR desc , inst_id desc
/
...
break on inicio skip 1 on report
set lines 250
set pages 500
col inicio for a20
col fin for a20
...
select inst_id,snap, inicio, prog, SQL_ID,EXEC_PLAN,SQL_EXEC_PLAN, exec, CPU, ELAP, PHY_RDS, BUFF_GET
from 
(
    select 
    sfst.instance_number inst_id,
    ...
    dhash.program prog,
    dhash.sql_id SQL_ID,
    sqlfst.PLAN_HASH_VALUE EXEC_PLAN,
    dhash.SQL_PLAN_HASH_VALUE SQL_EXEC_PLAN,
    ...
    and sfst.instance_number=&&nodo_1
    and sqlfst.instance_number=&&nodo_1
    and sqlfst.instance_number=&&nodo_1
   ...
   -- and '&FECHA' = to_char(sfst.end_INTERVAL_TIME, 'DD/MM/YY')
    ...
    where dhash.sql_id='&&SQL_ID'
    and sfst.instance_number=&&nodo_2
    and sqlfst.instance_number=&&nodo_2
    and sqlfst.instance_number=&&nodo_2
    and sfst.snap_id=dhash.snap_id
    and sqlfst.snap_id=sfst.snap_id
    and sqlfst.sql_id=dhash.sql_id
...
group by inst_id,snap,inicio, prog, SQL_ID,EXEC_PLAN,SQL_EXEC_PLAN, exec, CPU,ELAP, PHY_RDS,BUFF_GET
order by 2,1
/
...
break on sql_id skip 1;
      ...
      and sfst.instance_number=sqlfst1.instance_number
      and sqlfst1.instance_number=sqlsst2.instance_number
...
break on sql_id skip 1;
      ...
      select  INST_ID, SQL_ID, HASH , sum(EXECS) EXECS,sum(SS) SS, ROUND(AVG(SC_EXEC),6) SC_EXEC, min(inicio) "1 exec"
      FROM (
	    select 
      sfst.instance_number INST_ID,
	    to_char(sfst.BEGIN_INTERVAL_TIME, 'DD/MM/YY hh24:mi:ss') inicio,
      ...
      and sqlfst1.instance_number=sqlsst2.instance_number
	    ...
	    WHERE SQL_ID='&SQL_ID'
	    group by INST_ID, SQL_ID, HASH, inicio
	    order by  INST_ID, sql_id, hash
/
