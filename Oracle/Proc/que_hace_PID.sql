...
BEGIN
  ...
  select * into s from sys.gv_$session where sid  = v_sid;
  select * into p from sys.gv_$process where addr = s.paddr;
  ...
  for c1 in ( select * from sys.gv_$sqltext
  ...
  for c1 in ( select * from sys.gv_$session_wait where sid = s.sid) loop
    dbms_output.put_line(chr(9)||c1.state||': '||c1.event);
  end loop;

--  dbms_output.put_line('Connect Info:');
--  for c1 in ( select * from sys.gv_$session_connect_info where sid = s.sid) loop
